...
export default function Attendence() {
  ...
  const i2 = 'For 2 days or more – an application in writing along with a medical certificate from a registered medical \npractitioner must be submitted.'
  const array = [i1,i2];
            ...
            <ul style={{ listStyle: 'none' }} >
              ...
              {array.map((item, index) => (
                <li key={index} className='liststextinsidecmp1'>
                  ...
                  <pre className='listdatainsidecmp1'>{item}</pre>
                </li>
              ))}
            </ul>
          </div>
